...
class Neuron(object):
    ...
    def solveHHModel(self):
        ...
        for i in range(1,nt):
            ...
            kv1=dt*self.Euler_v(m,h,n,z,V,I)
            km1=alpham(V)*1.0/(alpham(V)+betam(V))
...
def alpham(V):
    out = 0.1*(V+35)/(1-exp(-(V+35)*1.0/10))
    return out

def betam(V):
    out = 4*exp(-(V+60)*1.0/18)
    return out

def alphah(V):
    out = 0.07*exp(-(V+58)*1.0/20)
    return out

def betah(V):
    out =1.0/(1+exp(-(V+28)*1.0/10))
    return out

def alphan(V):
    out = 0.01*(V+34)/(1-exp(-(V+34)*1.0/10))
    return out

def betan(V):
    out =0.125*exp(-(V+44)*1.0/80)
    return out
...
for Iinj in arange(7,20):      #Input current -uA
    hh = Neuron()
    print  Iinj
