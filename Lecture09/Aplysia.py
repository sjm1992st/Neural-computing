...
Synaptic_nmda2=Synaptic_NMDA(g_syn = -1.0/500,Ts=200,Tf=50, Is=-0.1, If=1.1,E_syn = 130)##g_syn,Ts,Tf, Is,If, E_syn, GNMDA = -1.0/500 ###[uM/(ms*mV)]ENMDA = 130  ##mV
Neuron_A = Neuron()
Neuron_B = Neuron()
Neuron_C = Neuron()
Neuron_D = Neuron()
...
for t in mtime:      #Input current -uA
    ...
    syn_AC=Synaptic_nmda1.cell_syn(t,A_v,C_v)
    syn_AE=Synaptic_nmda2.cell_syn(t,A_v,E_v)
    A_v = A_v+mstep*(A_kv)/C
    s_Ca1, W1=Synaptic_nmda1.computer_Ca(syn_AC, s_Ca1, 50, W1)
    #print s_Ca1
    mCa1.append(s_Ca1)
    list_w1.append(W1)
    s_Ca2, W2=Synaptic_nmda1.computer_Ca(syn_AE, s_Ca2, 50, W2)
    mCa2.append(s_Ca2)
    list_w2.append(W2)
    ...
    syn_BD=Synaptic_E1.cell_syn(t,B_v,D_v)
    syn_BE=Synaptic_E4.cell_syn(t,B_v,E_v)
    B_v = B_v+mstep*(B_kv)/C
    Neuron_B.solveHHModel_B()

    D_kv=Neuron_D.solveHHModel(0,D_v)
    syn_DC=Synaptic_E5.cell_syn(t,D_v,C_v)
    syn_DE=Synaptic_E6.cell_syn(t,D_v,E_v)
    D_v = D_v+mstep*(D_kv-syn_BD)/C
    #print syn_BD
    Neuron_D.solveHHModel_B()

    #print 'B',Neuron_B.mv[-1]
    C_kv=Neuron_C.solveHHModel(0,C_v)
    syn_CE=Synaptic_E2.cell_syn(t,C_v,E_v)
    C_v = C_v+mstep*(C_kv-syn_DC-syn_AC*W1)/C
    Neuron_C.solveHHModel_B()
   # print 'C',Neuron_C.mv[-1]
    E_kv=Neuron_E.solveHHModel(0,E_v)
    E_v = E_v+mstep*(E_kv-syn_BE-syn_AE*W2-syn_CE)/C
...
ax.set_xlabel('Time (ms)')
ax.set_ylabel('Voltage (mV)')
plt.plot(list(mtime),Neuron_C.mv,'k')


ax = fig.add_subplot(9,1,4)
ax.set_xlabel('Time (ms)')
ax.set_ylabel('Voltage (mV)')
plt.plot(list(mtime),Neuron_D.mv,'g')


ax = fig.add_subplot(9,1,5)
ax.set_xlabel('Time (ms)')
ax.set_ylabel('Voltage (mV)')
plt.plot(list(mtime),Neuron_E.mv,'c')
